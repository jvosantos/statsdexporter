...
import co.hold.domain.MetricType;
import co.hold.exceptions.InvalidStatsDLineException;
import co.hold.exceptions.MalformedMetricComponentException;
import co.hold.exceptions.UnknownMetricTypeException;
...
public class DefaultMetricMapper implements MetricMapper<DefaultEvent> {
    ...
    public DefaultMetricMapper() {
    }

    @Override
    public Flux<DefaultEvent> map(Collection<String> metricLines) {
        ...
        return Flux.merge(
                ...
                Flux.merge(metricLines
                        ...
                        .map(line -> this.processLine(line, new ArrayList<>())
                                .doOnError(e -> LOGGER.error("Error processing line", e))
                                .onErrorResume(throwable -> Mono.just(Collections.emptyList())))
                        .map(listMono -> listMono.flatMapMany(Flux::fromIterable))
                        .collect(toList()))
        );
    }
     ...
     * @return metric @Event
     */
    private Mono<List<DefaultEvent>> processLine(final String line, final List<DefaultEvent> events) {
